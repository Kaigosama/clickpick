...
async function run() {
  ...
  const now = Date.now();
  const seedTag = `verify-cancel-${now}`;
  let createdOrderId;
  ...
  try {
    const customer = await User.create({
      ...
      email: `verifier.cancel.customer.${now}@example.com`,
    ...
    const stall = await User.create({
      ...
      email: `verifier.cancel.stall.${now}@example.com`,
      password: 'test-password',
      role: 'stall_staff'
    });
    ...
    const menuItem = await MenuItem.create({
      name: `Verifier Cancel Item ${seedTag}`,
      description: 'Cancelled order no-deduction verifier item',
      price: 100,
      quantity: 10,
      category: 'main',
      isAvailable: true,
    ...
    if (!currentMenuItem || currentMenuItem.quantity !== 10) {
      throw new Error(`Expected quantity 10 after preparing status update, got ${currentMenuItem?.quantity}`);
    }

    await axios.put(`${BASE_API_URL}/orders/${createdOrderId}`, { status: 'cancelled' });
    currentMenuItem = await MenuItem.findById(menuItem._id);
    if (!currentMenuItem || currentMenuItem.quantity !== 10) {
      throw new Error(`Expected quantity 10 after cancellation, got ${currentMenuItem?.quantity}`);
    }

    await axios.put(`${BASE_API_URL}/orders/${createdOrderId}`, { status: 'cancelled' });
    currentMenuItem = await MenuItem.findById(menuItem._id);
    if (!currentMenuItem || currentMenuItem.quantity !== 10) {
      throw new Error(`Expected quantity to remain 10 on repeated cancellation, got ${currentMenuItem?.quantity}`);
    }

    console.log('✅ Verification passed: cancelled orders do not deduct inventory.');
